...
like `RESEARCH BAY`). The text color can be set per object; otherwise the
theme's `floorLabelColor` applies.
...
1. Create a map in Tiled: orthogonal, 16×16 tiles (the example uses 40×24).
