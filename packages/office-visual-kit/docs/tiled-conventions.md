...
boards/monitors as object sprites over the wall face; workstations as 2×2
desk blocks (monitor upper half, keyboard lower edge) with the back-facing
agent's spawn point at the block's bottom-center, feet ≈2 px above the next
tile row — the agent tucks under the desk and faces the screen; one empty
tile row between stacked workstations so nameplates have floor under them;
the status badge cleared above the monitor via `theme.statusBadgeOffsetY`;
cabinets/break area/infra grouped along walls; plants in seams and corners.
